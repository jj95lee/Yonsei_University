...
clc
%% data setting
Timedata = importdata("nSC= 10_nPlanes=1_f=4_AccessReport_3day.txt",'');
...
%% Plotting
figure(1)
histogram(disconnect_time,100) % 가로축: disconnect_time, 세로축: 불연속 횟수 

figure(2)
timeaa = datetime(jdtimespan,'ConvertFrom','juliandate'); % 타겟포인트를 지나는 위성의 수 
plot(timeaa,K)


figure(3); 
set(gca,'fontsize',12); hold on; grid on;
y = [1.91 1.35 2.9 4.36];
b = bar([0 1 2 3], y);
b.FaceColor = 'flat';
