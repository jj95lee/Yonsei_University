...
while sqrt(mu/sma)/sma*180/pi*t(j) <= 360
    ...
    if ang_in(j, 1) > 360 + 90 + alpha + 7.45
       ang_in(j, 1) = 0;
    elseif ang_in(j, 1) > 360
        ang_in(j, 1) = (sqrt(mu/sma)/sma*180/pi*t(j) - alpha) - 90;
    elseif 280.3 < ang_in(j, 1) && ang_in(j, 1)  < 323.5
        ang_in(j, 1) = 0;
    end
...
% Eclipse In & Out
ang_eclin = acosd(Re/(Re+H)) + 90 + 7.45 ;
ang_eclout = 270 - acosd(Re/(Re+H));

%% 
angledata = importdata("data1.txt");
k = angledata(1:length(angledata),1);
C = {};
